...
public class BattleShipFrame extends JFrame {
  ...
  private void placeOpponentShips() {
    ...
    placeShip(opponentBoard, 5, 1);
    placeShip(opponentBoard, 4, 2);
    placeShip(opponentBoard, 3, 3);
    placeShip(opponentBoard, 3, 4);
    placeShip(opponentBoard, 2, 5);
  }

  private void placeShip(int[][] onBoard, int shipSize, int shipNum) {
    if (DEBUG) {System.out.println("Looking to place a ship of size"+shipSize);}
    shipNum = 10*shipNum;
    boolean clear;
    int chooseRow;
    int chooseColumn;
